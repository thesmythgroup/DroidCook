...
public class Receiver extends BroadcastReceiver {
  ...
  public static interface IEvent {
    ...
    void broadcast(LocalBroadcastManager local);
    void broadcastSync(LocalBroadcastManager local);
  }
  ...
  public IntentFilter filter(Object obj) {
  ...
  public void register(Activity act, Object obj) {
    ...
    mObj = obj;
    act.registerReceiver(this, filter(obj));
  }

  public void register(LocalBroadcastManager local, Object obj) {
    if (mObj != null) {
      throw new RuntimeException("Already have registered object.");
    }
    mObj = obj;
    local.registerReceiver(this, filter(obj));
  }
  ...
  public void unregister(LocalBroadcastManager local) {
    local.unregisterReceiver(this);
    mObj = null;
    mEvents.clear();
  }
  ...
  static class EventHandler implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      if ("broadcast".equals(method.getName())) {
        if (args[0] instanceof LocalBroadcastManager) {
          ((LocalBroadcastManager) args[0]).sendBroadcast(new Intent(mAction).putExtras(mExtras));
        } else {
          ((Context) args[0]).sendBroadcast(new Intent(mAction).putExtras(mExtras));
        }
        return null;
      } else if ("broadcastSync".equals(method.getName())) {
        ((LocalBroadcastManager) args[0]).sendBroadcastSync(new Intent(mAction).putExtras(mExtras));
